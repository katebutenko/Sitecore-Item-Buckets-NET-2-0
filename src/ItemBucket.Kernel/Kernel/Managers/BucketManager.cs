...
namespace Sitecore.ItemBucket.Kernel.Managers
{
    ...
    public static class BucketManager
    {
        ...
        {
            return item.Axes.GetAncestors().Where(IsBucket).DefaultIfEmpty(Context.ContentDatabase.GetItem(Context.Site.RootPath)).First();
        }
        ...
        {
            return item.Axes.GetAncestors().Where(IsBucket).DefaultIfEmpty(item.Parent).First();
        }
        ...
        {
            return item.IsABucket() ? item : item.Axes.GetAncestors().Where(IsBucket).DefaultIfEmpty(Context.ContentDatabase.GetItem(Context.Site.RootPath)).First();
        }
        ...
        public static string GetContextIndex(Item item)
        {
            if (item.IsNotNull())
            {
                ...
                foreach (var index in from index in SearchHelper.GetIndexes()
                                      let indexConfigurationNode =
                                          Configuration.Factory.GetConfigNode(
